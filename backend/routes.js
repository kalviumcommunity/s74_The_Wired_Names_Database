...
router.post("/", async (req, res) => {
  try {
    const { username, origin, meaning, created_by } = req.body;
    ...
    const newUser = new User({ username, origin, meaning, created_by });
    await newUser.save();
...
router.put(
  ...
  [
    ...
    body("meaning")
      .optional()
      .isLength({ min: 5 })
      .withMessage("Meaning must be at least 5 characters long"),
  ],
  async (req, res) => {
    const { id } = req.params;
    ...
    // Validation for created_by during update
...
router.get("/creator/:creatorId", async (req, res) => {
  try {
    ...
    const users = await User.find({ created_by: creatorId });
    res.json(users);
  } catch (error) {
